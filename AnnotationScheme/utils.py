...
def _enable_path_completion():
    """
    Activate TAB-completion for file-system paths *without* duplicating
    the already-typed prefix (e.g. '../../Ne' ➜ '../../NewMaintenanceData').
    Works on Unix / macOS.  On Windows install `pyreadline3` first.
    """
    def _path_completer(text, state):
        """
        `text`  : the *current word* that readline wants to complete
                  (everything after the last whitespace AND '/'!)
        `state` : 0,1,2,…  successive matches
        We return ONLY the suffix that has to replace <text>.
        """

        # Where in the full line does this word start?
        buf      = readline.get_line_buffer()
        begidx   = readline.get_begidx()          # index of first char in <text>
        dir_part = os.path.expanduser(buf[:begidx]) or "."  # '../../'  part
        # `text` may still hold a partial subdir (e.g. 'Ne' or 'Ne/An')
        search_dir, prefix = os.path.split(os.path.join(dir_part, text))
        if search_dir == "":                      # e.g. just 'Ne'
            search_dir = "."

        # Collect matches only once per TAB press ----------------------
        if state == 0:
            try:
                ...
                _matches[:] = [
                    entry + ("/" if os.path.isdir(os.path.join(search_dir, entry)) else "")
                    for entry in entries
                    if entry.startswith(prefix)
                ]
                _matches.sort()

        # Return the <state>-th match, but **only the part after dir_part**
        try:
            match = _matches[state]
            # We must replace <text>, so strip the already-typed prefix
            return match
        except IndexError:
            return None
    ...
    try:
        ...
        readline.set_completer(old_comp)          # restore previous completer


def prompt_path(question="Path: ", default=None):
    """
    Ask for a filesystem path with TAB completion.
    Returns the expanded, absolute path.
    """
    with _enable_path_completion():
